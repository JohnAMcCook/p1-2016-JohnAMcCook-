...
public class TrafficAnimation extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		Color road = new Color(9, 58, 74);
		g.setColor(road);
		g.fillRect(0, height / 3, width, height / 3);
		g.setColor(Color.gray);
		g.fillRect(xOffset, height / 5, width / 3, height / 3);
		g.setColor(Color.green);
		g.fillRect(xOffset + width / 5, height / 3 + height / 20, squareSide, squareSide);
		g.setColor(Color.blue);
		g.setFont(new Font("Helvetica", Font.PLAIN, height / 40));
		g.drawString("Don't talk to me...", xOffset + width / 5, height / 2);
